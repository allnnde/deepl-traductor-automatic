...
// Read config file
const CONFIG = JSON.parse(readFileSync("./db-extractor/db-extractor-config.json", "utf-8"));
...
if (!existsSync(CONFIG.filePaths.db)) {
    mkdirSync(CONFIG.filePaths.db,{recursive:true});
}

// Pull current pf2 release and extract db files
if (!existsSync(CONFIG.filePaths.actor)) {
    mkdirSync(CONFIG.filePaths.actor,{recursive:true});
}

// Pull current pf2 release and extract db files
if (!existsSync(CONFIG.filePaths.crowdin)) {
    mkdirSync(CONFIG.filePaths.crowdin,{recursive:true});
}

// Pull current pf2 release and extract db files
if (!existsSync(CONFIG.filePaths.i18n)) {
    mkdirSync(CONFIG.filePaths.i18n,{recursive:true});
}
...
function extractPack(packName, packConfig, dbFiles) {
    ...
    if (dbFiles.includes(`${packName}.db`)) {
        ...
        writeFile(
            `${CONFIG.filePaths[packConfig.savePath]}/${compendiumName}.json`,
        ...
        console.log(`Extracted pack: ${compendiumName}`);
    }
}
